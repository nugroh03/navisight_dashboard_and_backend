...
const updateProject = async ({ id, name }: { id: string; name: string }): Promise<Project> => {
...
export default function ProjectsPage() {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (editingProject) {
      updateMutation.mutate({ id: editingProject.id, name: projectName.trim() });
    } else {
    ...
    return (
      <div className="space-y-6">
        <div className="card border-[var(--color-border)] bg-white p-8 shadow-lg">
          ...
          <p className="mt-2 text-red-600">Error loading projects: {(error as Error).message}</p>
        </div>
      </div>
    );
  }
      ...
      <div className="card border-[var(--color-border)] bg-white p-8 shadow-lg">
        <div className="flex items-center justify-between">
          <div>
            ...
            <h1 className="mt-3 text-2xl font-semibold text-[var(--color-text)]">Projects</h1>
      ...
      <div className="card border-[var(--color-border)] bg-white p-8 shadow-lg">
        {isLoading ? (
          ...
          <div className="text-center py-12">
            ...
            <h3 className="text-lg font-medium text-[var(--color-text)] mb-2">No projects yet</h3>
            <p className="text-[var(--color-muted)] mb-4">
              Create your first project to get started with NAVISIGHT
            </p>
            <button
              onClick={() => handleOpenModal()}
        ...
        <div className="fixed inset-0 bg-slate-900/30 flex items-center justify-center p-4 z-50">
          <div className="bg-white rounded-lg shadow-xl max-w-md w-full p-6">
            ...
            <form onSubmit={handleSubmit}>
              <div className="space-y-4">
                <div>
                  <label htmlFor="projectName" className="block text-sm font-medium text-[var(--color-text)] mb-1">
                    Project Name
                  </label>
                  <input
                    type="text"
